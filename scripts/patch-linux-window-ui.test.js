...
test("Computer Use availability descriptor matches the current settings bundle name", () => {
  ...
  const descriptor = descriptors.find(({ id }) => id === "linux-computer-use-ui-availability");
  const pluginsDescriptor = descriptors.find(({ id }) => id === "linux-computer-use-plugins-page-availability");
  const installFlowDescriptor = descriptors.find(({ id }) => id === "linux-computer-use-install-flow");

  assert.match("computer-use-settings-DsM_pz8i.js", descriptor.pattern);
  assert.match("plugins-page-gTuUJ93E.js", pluginsDescriptor.pattern);
  assert.match(
  ...
  assert.equal(installFlowDescriptor.ciPolicy, "required-upstream");
...
test("rejects current Computer Use host-platform drift byte-identically", () => {
  ...
  const hostDescriptor =
    require("./patches/core/all-linux/webview/computer-use-ui/patch.js")
      .find(({ id }) => id === "linux-computer-use-host-platform");
...
test("loads July 24 Computer Use plugin details through the expanded availability chain", () => {
  const source =
    "function n4i(e){let t=cache(31),{hostId:n,marketplacePath:r,pluginName:i,remoteMarketplaceName:a,enabled:o}=e," +
    "s=o===void 0?!0:o,c=n??`local`,l;t[0]===c?l=t[1]:(l={hostId:c},t[0]=c,t[1]=l);" +
    "let u=hostReady(l),d=environment(),f;t[2]===i?f=t[3]:(f=i!=null&&isAvailabilityGated(i),t[2]=i,t[3]=f);" +
    "let p=f,m;t[4]!==c||t[5]!==p?(m={enabled:p,hostId:c},t[4]=c,t[5]=p,t[6]=m):m=t[6];" +
    "let h=useComputerUseAvailability(m),g=(r!=null||a!=null)&&i!=null,_=u&&s&&g&&p&&h.isLoading,v=u&&s&&g&&(!p||h.available);" +
    "let b=async()=>{if(i==null)throw Error(`plugin detail query requires pluginName`);return read(`read-plugin`,{hostId:c,pluginName:i})};" +
    "return useQuery({queryFn:b,enabled:v})}";
...
test("rejects current Computer Use plugin detail drift byte-identically", () => {
  ...
  const installFlowDescriptor =
    require("./patches/core/all-linux/webview/computer-use-ui/patch.js")
      .find(({ id }) => id === "linux-computer-use-install-flow");
  ...
  try {
    ...
    assert.deepEqual(
      ...
      [
        ...
        { name: "linux-computer-use-install-flow", status: "failed-required" },
      ],
    );
  } finally {
    fs.rmSync(tempRoot, { recursive: true, force: true });
...
test("reconciles the global Plugins page to the installed Computer Use plugin", () => {
  const source =
  ...
  const result = vm.runInNewContext(
    `${patched};({browse:Ei.filter(e=>e.plugin.name===\`computer-use\`),detail:qi,featured:Ai.plugins.filter(e=>e.plugin.name===\`computer-use\`),unavailable:[...Xi]})`,
  );

  assert.equal(result.browse.length, 1);
  assert.equal(result.browse[0].plugin.id, "computer-use@openai-bundled");
  assert.equal(result.detail.plugin.id, "computer-use@openai-bundled");
  assert.equal(result.featured[0].plugin.id, "computer-use@openai-bundled");
  assert.deepEqual(Array.from(result.unavailable), ["other-missing"]);
});

test("reconciles the latest Plugins page collection to bundled Computer Use", () => {
  const source =
  ...
  const patched = applyPatchTwice(applyLinuxComputerUsePluginsPageAvailabilityPatch, source);
  const result = vm.runInNewContext(
    `${patched};({browse:bi.filter(e=>e.plugin.name===\`computer-use\`),featured:Ai.plugins.filter(e=>e.plugin.name===\`computer-use\`),unavailable:[...Ji]})`,
  );
