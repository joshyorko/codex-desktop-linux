...
test("patches Linux Chrome Beta and Unstable support into bundled Chrome plugin scripts", () => {
  ...
  try {
    writeScript(
      ...
      '    const fs = require("node:fs");     const os = require("node:os");     const path = require("node:path");     function getNativeHostManifestDetails(expectedHostName) {       if (process.platform === "win32") {         const registryKey = `${WINDOWS_NATIVE_HOST_REGISTRY_KEY_PREFIX}\${expectedHostName}`;         const registryManifestPath = readWindowsRegistryDefaultValue(registryKey);         return {           manifestPath: registryManifestPath || getDefaultWindowsManifestPath(),           registryKey,           registryManifestPath,           registryKeyExists: registryManifestPath != null,         };       }       if (process.platform === "linux") return {         manifestPath: path.join(           os.homedir(),           ".config",           "google-chrome",           "NativeMessagingHosts",           `${expectedHostName}.json`,         ),         registryKey: null,         registryManifestPath: null,         registryKeyExists: null,       };       throw new Error(`Unsupported platform native host manifest check: ${process.platform}. This script supports macOS and Windows.`);     }',
    );
