...
function applyMainBundlePatch(source) {
  ...
  const helper = [
    ...
    `function codexLinuxReadAloudCommandDirs(){let e=(process.env.PATH||\`\`).split(${pathVar}.delimiter).filter(Boolean),t=(process.env.CODEX_LINUX_READ_ALOUD_COMMAND_DIRS||\`\`).split(${pathVar}.delimiter).filter(Boolean),n=process.env.CODEX_LINUX_READ_ALOUD_STANDARD_COMMAND_DIRS?.trim().toLowerCase(),r=[\`/usr/local/bin\`,\`/usr/bin\`,\`/bin\`,\`/usr/sbin\`,\`/sbin\`,\`/home/linuxbrew/.linuxbrew/bin\`];return n===\`0\`||n===\`false\`||n===\`off\`||n===\`no\`?[...e,...t]:[...e,...t,...r]}`,
    `function codexLinuxReadAloudCommandPath(command){if(!command)return null;if(command.includes(\`/\`))try{return ${fsVar}.existsSync(command)&&(${fsVar}.accessSync(command,${fsVar}.constants.X_OK),!0)?command:null}catch{return null};for(let e of codexLinuxReadAloudCommandDirs()){let t=${pathVar}.join(e,command);try{if(${fsVar}.existsSync(t)&&(${fsVar}.accessSync(t,${fsVar}.constants.X_OK),!0))return t}catch{}}return null}`,
    `function codexLinuxReadAloudCommandExists(command){return codexLinuxReadAloudCommandPath(command)!=null}`,
...
function applyLinuxDesktopSettingsPatch(source) {
  ...
  if (!patched.includes("$.jsx(LinuxReadAloudSettings,{}),$.jsx(LinuxSettingsSection,{title:\"Build\"")) {
    patched = patched.replace(
      '$.jsx(LinuxSettingsSection,{title:"Build",children:$.jsx(LinuxBuildInfoPanel,{})})',
      '$.jsx(LinuxReadAloudSettings,{}),$.jsx(LinuxSettingsSection,{title:"Build",children:$.jsx(LinuxBuildInfoPanel,{})})',
    );
  }
